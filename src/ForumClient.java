...
public class ForumClient extends Thread implements Runnable {
    ...
    public void setPost(ArrayList<String> post, boolean send) {
        this.post = post;
        if (send) {
            try {
                if (!post.isEmpty()) {
            ...
            if (message != null) {
                switch (message.getListName()) {
                    ...
                    case "Response" -> {
                        ...
                        if (studentName.equals(username) || (post != null && !post.isEmpty() && post.get(0)
                                .substring(post.get(0).indexOf(" ") + 1, post.get(0).indexOf(":")).matches(message
                                        .getContents().get(0).substring(message.getContents().get(0).indexOf(" ") + 1,
                                                message.getContents().get(0).indexOf(":"))))) {
                            if (modifier.equals("GUI")) {
