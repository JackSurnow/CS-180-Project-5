...
public class ForumClient extends Thread implements Runnable {
    ...
    public synchronized void setPosts(ArrayList<String> posts1, boolean send) {
        this.posts = posts1;
        if (send) {
    ...
    public synchronized void setAccounts(ArrayList<String> accounts1, boolean send) {
    ...
    public synchronized void setVotes(ArrayList<String> votes1, boolean send) {
        this.votes = votes1;
        if (send) {
    ...
    public synchronized void setPost(ArrayList<String> post1, boolean send) {
        this.post = post1;
        if (send) {
            try {
                if (!post1.isEmpty()) {
                    String fileName = String.format("Post%d.txt", Integer.parseInt(post1.get(0)
                            .substring(post1.get(0).indexOf(" ") + 1, post1.get(0).indexOf(":"))));
                    forumClient.sendPost(fileName, post1);
                }
    ...
    public static void main(String[] args) {
        try {
            ...
            forumClient1.setOs(new ObjectOutputStream(socket.getOutputStream()));
            forumClient1.setIs(new ObjectInputStream(socket.getInputStream()));

            while (ForumClient.getPostsFile() == null || ForumClient.getAccountsFile() == null || ForumClient
                    .getVotesFile() == null || forumClient1.getPosts() == null || forumClient1.getAccounts() == null
                    || forumClient1.getVotes() == null) {
                forumClient1.receive();
            }

            SwingUtilities.invokeLater(forumClient1);
    ...
    public synchronized void sendUsername(String username1) {
        try {
            os.writeObject(new Message("Username", username1));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public boolean guiSort(ArrayList<String> sort1) {
        if (sort1 != null && !sort1.isEmpty()) {
            textArea.setText("");
            for (int i = 0; i < sort1.size(); i++) {
                textArea.append(sort1.get(i));
                if (i < sort1.size() - 1) {
                    textArea.append("\n");
                }
            }
    ...
    ActionListener actionListener = new ActionListener() {
        ...
        public void actionPerformed(ActionEvent e) {
            if (e.getSource() == viewRepliesByVotes || e.getSource() == most) {
                ...
                Reply reply1 = new Reply(forumClient);
                reply1.replyManual(post);
