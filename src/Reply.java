...
public class Reply {
    ...
    public Reply(ForumClient forumClient) {
        this.forumClient = forumClient;
        this.previous = 0;
        this.replyNumber = null;
    }

    public void replyManual(ArrayList<String> post) {
        ...
        if (replyNumber != null && !replyNumber.isEmpty() && (valid || replyNumber.equals("0"))) {
    ...
    public void replyImport(ArrayList<String> post, String parentReply, String message, boolean first) {
    ...
    public void reply(ArrayList<String> post, String message) {
                ...
                if (post.get(post.size() - 1).substring(0, post.get(post.size() - 1).indexOf(":")).contains(".")) {
                    ...
                    try {
                        replyNumber = String.format("%d", Integer.parseInt(post.get(post.size() - 1).substring(
                                post.get(post.size() - 1).indexOf("y") + 2, post.get(post.size() - 1)
                                        .indexOf("."))) + 1);
        ...
        StringBuilder output = new StringBuilder(String.format("Reply %s: %s said \"%s\" at %s",
                replyNumber, forumClient.getUsername(), message, Time.time()));
